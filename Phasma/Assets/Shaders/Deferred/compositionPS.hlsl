...
PS_OUTPUT mainPS(PS_INPUT input)
{
    ...
    if (depth == 0.0)
    {
        ...
        if (cb_effects3.y > 0.5)
        {
            ...
            if (cb_effects1.y > 0.5)
                output.color.xyz += VolumetricLighting(cb_sun, wolrdPos, input.uv, cb_cascades[0], fogFactor);
        }
    ...
    float3 fragColor = 0.0;// 0.1 * material.albedo.xyz;
    ...
    output.color = float4(fragColor, albedo.a) + Emission.Sample(sampler_Emission, input.uv);
