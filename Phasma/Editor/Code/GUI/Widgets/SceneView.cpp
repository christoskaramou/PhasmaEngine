...
namespace pe
{
    ...
    void SceneView::DrawOrientationGizmo(const ImVec2 &imageMin, const ImVec2 &imageSize)
    {
        ...
        const ui::OrientationGizmoResult gizmo = ui::DrawOrientationGizmo(camera, imageMin, imageSize);
        s_orientationGizmoHovered = gizmo.hovered;
        s_orientationGizmoActive = gizmo.active;
        if (gizmo.clicked)
            GizmoUtils::ApplyCameraOrientationFromDirection(camera, gizmo.direction);
    }
