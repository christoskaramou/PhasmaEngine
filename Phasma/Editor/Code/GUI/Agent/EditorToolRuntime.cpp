...
namespace pe
{
    namespace
    {
        ...
        namespace mapoverlay
        {
            ...
            inline void Tick(uint8_t *img, int W, int H, int x, int y, int radius, Col c)
            {
                FillRect(img, W, H, x - radius, y - radius, x + radius, y + radius, c);
            }
        } // namespace mapoverlay
    ...
    std::string EditorToolRuntime::CreateSpriteNode(const std::string &argsJson) const
    {
        ...
        QueueAction([state, args]()
            ...
            else
            {
                ...
                SpriteAuthoring::ReadSpriteOptions(args, options);
                ...
                if (!sprite.error.empty())
                {
                    ...
                    else
                    {
                        ...
                        if (!SpriteAuthoring::ApplySpriteJsonTransform(sc, sprite.node, args, meshSlot, transformError))
                        state->result = JsonObj({{"error", JsonStr(transformError)}});
                    else
                        state->result = SpriteAuthoring::SpriteResultJson(sc, sprite).dump();
                }
            }
            {
                std::lock_guard lock(state->mtx);
        ...
        QueueAction([state, nodeId]()
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (sprite)
                        result["sprite"] = SpriteAuthoring::SpriteComponentJson(*sprite);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            std::string transformError;
                            if (!SpriteAuthoring::ApplySpriteJsonTransform(sc, node, args, meshSlot, transformError))
                                state->result = JsonObj({{"error", JsonStr(transformError)}});
                            else
                                state->result = SpriteAuthoring::SpriteResultJson(sc, sprite).dump();
                        }
    ...
    std::string EditorToolRuntime::ReloadSpriteMetadata(const std::string &argsJson) const
    {
        ...
        QueueAction([state, nodeId]()
            ...
            else
            {
                ...
                {
                    const NodeSpriteComponent *sprite = sc.GetSpriteComponent(node);
                    state->result = nlohmann::json{{"status", "ok"}, {"node", MakeNodeId(node)}, {"sprite", sprite ? SpriteAuthoring::SpriteComponentJson(*sprite) : nlohmann::json(nullptr)}}.dump();
                }
            }
            {
                std::lock_guard lock(state->mtx);
                ...
                else
                {
                    ...
                    else
                    {
                        if (NodeSpriteComponent *sprite = sc.GetSpriteComponent(node))
                        {
                            ...
                            state->result = nlohmann::json{{"status", "ok"}, {"node", MakeNodeId(node)}, {"sprite", SpriteAuthoring::SpriteComponentJson(*sprite)}}.dump();
                        }
                    }
                }
            }
    ...
    std::string EditorToolRuntime::ReadSpriteMetadata(const std::string &argsJson) const
    {
        ...
        const std::filesystem::path path = SpriteAuthoring::ResolveSpriteMetadataPath(pathArg);
    ...
    std::string EditorToolRuntime::ValidateSpriteAsset(const std::string &argsJson) const
    {
        ...
        if (nodeId.empty())
            return SpriteAuthoring::ValidateSpriteMetadataAsset(pathArg).dump();
        ...
        QueueAction([state, args, nodeId, pathArg]()
            ...
            if (!renderer)
            {
                result = SpriteAuthoring::ValidateSpriteMetadataAsset(pathArg);
            ...
            else
            {
                ...
                result = SpriteAuthoring::ValidateSpriteMetadataAsset(resolvedPathArg);
    ...
    std::string EditorToolRuntime::WriteSpriteMetadata(const std::string &argsJson) const
    {
        ...
        if ((frames.empty() || args.value("generate_grid", false)) && !grid.empty())
            frames = SpriteAuthoring::GenerateSpriteFramesJson(grid, imageW, imageH);
        ...
        result["status"] = "ok";
        result["path"] = PathUtf8(path);
        result["asset_path"] = SpriteAuthoring::AssetRelativePathForTool(path);
    ...
    std::string EditorToolRuntime::AddSpriteFrame(const std::string &argsJson) const
    {
        ...
        nlohmann::json root;
        std::string error;
        if (!SpriteAuthoring::LoadSpriteMetadataForEdit(args.value("path", args.value("metadata_path", "")), path, root, error))
            return nlohmann::json{{"error", error}}.dump();

        nlohmann::json frame;
        if (!SpriteAuthoring::ApplySpriteFrameFields(frame, args, true, error))
            return nlohmann::json{{"error", error}}.dump();
        ...
        SpriteAuthoring::ClampSpriteClips(root);

        if (!SpriteAuthoring::SaveJsonAsset(path, root, error))
            return nlohmann::json{{"error", error}}.dump();
        nlohmann::json result = SpriteAuthoring::SpriteMetadataEditResult(path, root);
    ...
    std::string EditorToolRuntime::UpdateSpriteFrame(const std::string &argsJson) const
    {
        ...
        const int index = SpriteAuthoring::FindNamedOrIndexedItem(root["frames"], args, {"index", "frame", "frame_index"}, {"frame_name"}, error);
        if (index < 0)
            return nlohmann::json{{"error", error}}.dump();

        if (!SpriteAuthoring::ApplySpriteFrameFields(root["frames"][index], args, false, error))
            return nlohmann::json{{"error", error}}.dump();
        SpriteAuthoring::ClampSpriteClips(root);
    ...
    std::string EditorToolRuntime::RemoveSpriteFrame(const std::string &argsJson) const
    {
        ...
        const int index = SpriteAuthoring::FindNamedOrIndexedItem(root["frames"], args, {"index", "frame", "frame_index"}, {"name", "frame_name"}, error);
        if (index < 0)
            return nlohmann::json{{"error", error}}.dump();
    ...
    std::string EditorToolRuntime::ReorderSpriteFrames(const std::string &argsJson) const
    {
        ...
        {
            const int from = SpriteAuthoring::FindNamedOrIndexedItem(frames,
                                                                     args,
    ...
    std::string EditorToolRuntime::AddSpriteClip(const std::string &argsJson) const
    {
        ...
        nlohmann::json clip;
        if (!SpriteAuthoring::ApplySpriteClipFields(clip, args, true, error))
            return nlohmann::json{{"error", error}}.dump();
    ...
    std::string EditorToolRuntime::UpdateSpriteClip(const std::string &argsJson) const
    {
        ...
        const int index = SpriteAuthoring::FindNamedOrIndexedItem(root["clips"], args, {"index", "clip", "clip_index"}, {"clip_name"}, error);
        if (index < 0)
            return nlohmann::json{{"error", error}}.dump();

        if (!SpriteAuthoring::ApplySpriteClipFields(root["clips"][index], args, false, error))
            return nlohmann::json{{"error", error}}.dump();
        SpriteAuthoring::ClampSpriteClips(root);
    ...
    std::string EditorToolRuntime::RemoveSpriteClip(const std::string &argsJson) const
    {
        ...
        const int index = SpriteAuthoring::FindNamedOrIndexedItem(root["clips"], args, {"index", "clip", "clip_index"}, {"name", "clip_name"}, error);
        if (index < 0)
            return nlohmann::json{{"error", error}}.dump();
    ...
    std::string EditorToolRuntime::ReadSpriteSheet(const std::string &argsJson) const
    {
        ...
        const std::filesystem::path path = SpriteAuthoring::ResolveSpriteSheetPath(pathArg);
        if (!pmcp::IsPathSafe(path.string(), Path::Assets))
            return R"({"error":"path must stay inside Assets/"})";
    ...
    std::string EditorToolRuntime::WriteSpriteSheet(const std::string &argsJson) const
    {
        ...
        for (const nlohmann::json &entry : args["images"])
        {
            ...
            std::string error;
            if (!SpriteAuthoring::ReadSpriteSheetImageEntry(entry, normalizedEntry, error))
    ...
    std::string EditorToolRuntime::CreateSpriteFromSheet(const std::string &argsJson) const
    {
        ...
        const std::filesystem::path sheetPath = SpriteAuthoring::ResolveSpriteSheetPath(sheetArg);
        ...
        std::string error;
        if (!SpriteAuthoring::ReadSpriteSheetImageAt(sheet, sheetPath, imageIndex, imagePath, imageLabel, error))
            return nlohmann::json{{"error", error}}.dump();
        ...
        {
            metadataPath = SpriteAuthoring::ResolveSpriteMetadataPath(metadataArg);
        }
        else
        {
        ...
        metadata["image"] = SpriteAuthoring::AssetRelativePathForTool(imagePath);
        metadata["sheet"] = SpriteAuthoring::AssetRelativePathForTool(sheetPath);
        ...
        if (!SpriteAuthoring::SaveJsonAsset(metadataPath, metadata, error))
            return nlohmann::json{{"error", error}}.dump();

        nlohmann::json result = SpriteAuthoring::SpriteMetadataEditResult(metadataPath, metadata);
        result["sheet_path"] = PathUtf8(sheetPath);
        result["sheet_asset_path"] = SpriteAuthoring::AssetRelativePathForTool(sheetPath);
        result["image_path"] = PathUtf8(imagePath);
        result["image_asset_path"] = SpriteAuthoring::AssetRelativePathForTool(imagePath);
        ...
        QueueAction([state, args, metadataPath, result]()
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        state->result = result;
                        state->result["node_result"] = SpriteAuthoring::SpriteResultJson(sc, sprite);
                    }
                }
            }
            {
                ...
                if (cam && !cap.contains("error"))
                {
                    ...
                    if (annotate && !pngPath.empty())
                    {
                        ...
                        if (img && iw > 0 && ih > 0)
                        {
                            ...
                            const int tickR = std::max(2, iw / 480);
                            ...
                            for (float wx = std::ceil(wmin.x / step) * step; wx <= wmax.x + 1e-3f; wx += step)
                            {
                                ...
                                mapoverlay::Tick(img, iw, ih, (int)a.x + 2, (int)a.y + 2, tickR, labelC);
                            }
    ...
    // Occlusion-exact object-id perception from the active camera (Phase 2c). The GPU work lives
    // in pe::DecodeCameraView (shared with the Lua scene.decode_view binding); this wrapper marshals
    // it onto the render thread and serializes the result struct to JSON.
    std::string EditorToolRuntime::DecodeCameraView(const std::string &argsJson) const
    {
        ...
        QueueAction([state, minPixels]()
            ...
            try
            {
                ...
                CameraViewResult r = pe::DecodeCameraView(scene, depth, static_cast<uint32_t>(minPixels));
                if (!r.valid)
                {
                    state->result = JsonObj({{"error", JsonStr(r.error.empty() ? "decode failed" : r.error)}});
                    finish();
                    return;
                }
                ...
                {
                    NodeId *node = (n.nodeIndex >= 0 && n.nodeIndex < static_cast<int>(scene.GetNodeCount()))
                                       ? scene.GetNodeId(static_cast<uint32_t>(n.nodeIndex))
                                       : nullptr;
                    if (!node)
                        continue;
                    nlohmann::json item;
                    ...
                    item["screen_box_px"] = nlohmann::json::array({n.minX, n.minY, n.maxX, n.maxY});
                    item["visible_pixels"] = n.visiblePixels;
                    item["nearest_ndc_depth"] = n.nearestNdcDepth;
                    item["distance"] = std::isfinite(n.distance) ? nlohmann::json(n.distance) : nlohmann::json(nullptr);
                    nodes.push_back(std::move(item));
                }

                nlohmann::json result;
                result["hit"] = !nodes.empty();
                result["image"] = {{"width", r.width}, {"height", r.height}};
                result["camera"] = {
                    {"position", Vec3Json(r.cameraPosition)},
                    {"view_projection", Mat4FlatJson(r.viewProjection)},
                };
    ...
    // Exact per-pixel pick for the decode_camera_view image (Phase 2c) — the perspective analogue
    // of pick_map_point. The GPU work lives in pe::PickCameraPixel (shared with the Lua
    // scene.pick_view_pixel binding); this wrapper marshals it onto the render thread and
    // serializes the result struct to JSON.
    std::string EditorToolRuntime::PickCameraPoint(const std::string &argsJson) const
    {
        auto args = nlohmann::json::parse(argsJson.empty() ? "{}" : argsJson, nullptr, false);
        if (args.is_discarded() || !args.is_object())
            return R"({"error":"invalid args json"})";
        if (!args.contains("x") || !args.contains("y") || !args["x"].is_number() || !args["y"].is_number())
            return R"({"error":"x and y (pixel coords in the decode_camera_view image) are required"})";

        const int reqX = args["x"].get<int>();
        const int reqY = args["y"].get<int>();
        const float groundY = args.value("ground_y", 0.0f);
        ...
        QueueAction([state, reqX, reqY, groundY]()
                    {
            auto finish = [&]()
            {
                {
            ...
            try
            {
                ...
                Image *depth = renderer->GetDepthStencilTarget("depthStencil");
                Scene &scene = renderer->GetScene();
                CameraPickResult r = pe::PickCameraPixel(scene, depth, reqX, reqY, groundY);
                if (!r.valid)
                {
                    nlohmann::json err;
                    err["error"] = r.error.empty() ? "pick failed" : r.error;
                    if (r.width && r.height)
                        err["image"] = {{"width", r.width}, {"height", r.height}};
                    state->result = err.dump();
                    finish();
                    return;
                }

                nlohmann::json result;
                result["pixel"] = nlohmann::json::array({reqX, reqY});
                result["image"] = {{"width", r.width}, {"height", r.height}};
                result["hit"] = r.hit;
                if (r.hit)
                {
                    result["world_hit"] = Vec3Json(r.worldHit);
                    result["nearest_ndc_depth"] = r.ndcDepth;
                    result["distance"] = std::isfinite(r.distance) ? nlohmann::json(r.distance) : nlohmann::json(nullptr);
                    if (r.nodeIndex >= 0 && r.nodeIndex < static_cast<int>(scene.GetNodeCount()))
                    {
                        if (NodeId *node = scene.GetNodeId(static_cast<uint32_t>(r.nodeIndex)))
                        {
                            result["node_id"] = MakeNodeId(node);
                            result["node_name"] = scene.GetNodeName(node);
                        }
                    }
                }
                else
                {
                    if (r.hasGroundPoint)
                        result["ground_point"] = nlohmann::json::array({r.groundPoint.x, r.groundPoint.y, r.groundPoint.z});
                    result["note"] = "no object at that pixel (sky/empty); ground_point is the camera ray intersected with Y=ground_y if it crosses the plane";
                }
                state->result = result.dump();
            }
            catch (const std::exception &e)
            ...
            {
                state->result = R"({"error":"unknown pick_camera_point failure"})";
            }
        ...
        if (!state->cv.wait_for(lock, std::chrono::seconds(30), [&]
                                { return state->done; }))
            return R"({"error":"timeout picking camera point"})";
        return state->result;
    }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (clear)
                                {
                                    image = ModelAsset::DefaultTextureForSlot(textureSlot);
