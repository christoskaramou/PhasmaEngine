...
namespace pe
{
    ...
    void DepthPyramidPass::AcquireTargets()
    {
        ...
        if (!m_pyramid)
            m_pyramid = rs->CreateRenderTarget("depthPyramid",
    ...
    void DepthPyramidPass::UpdatePassInfo()
    {
        // Reduce pipeline: previous pyramid level read as a storage image -> base m_passInfo.
        m_passInfo->name = "DepthPyramidReduce_pipeline";
        ...
        m_copyPassInfo->name = "DepthPyramidCopy_pipeline";
    ...
    void DepthPyramidPass::CreateUniforms(CommandBuffer *cmd)
    {
        (void)cmd;
    ...
    void DepthPyramidPass::UpdateDescriptorSets()
    {
        ...
        {
            DestroyDescriptors();
            m_copySet = Descriptor::Create(copyBindings, PE_SHADER_STAGE_COMPUTE, false, "DepthPyramidCopy_descriptor");
            ...
            for (uint32_t i = 0; i < reduceCount; ++i)
                m_reduceSets[i] = Descriptor::Create(reduceBindings, PE_SHADER_STAGE_COMPUTE, false,
                                                     "DepthPyramidReduce_descriptor_" + std::to_string(i));
        }
    ...
    void DepthPyramidPass::DeclareInputs(RGBuilder &builder)
    {
        builder.ReadCompute(m_depth);
        builder.WriteCompute(m_pyramid);
    }

    void DepthPyramidPass::DeclareOutputs(RGBuilder &builder)
    {
        (void)builder;
        // Deliberately declare NO output transition for the pyramid. WriteCompute (above)
        // already leaves the tracked state at GENERAL / COMPUTE / SHADER_WRITE and registers
        // this pass as the pyramid's producer, so the OcclusionCullingPass consumer's
        // ReadCompute emits the real GENERAL->SHADER_READ_ONLY (WRITE->READ) barrier itself.
        // Pre-marking the state to SHADER_READ here via OutputCustom only mutates tracked
        // state (no real barrier) and would make that consumer barrier a same-state no-op
        // (Image::Barriers skips requestRead && previousRead && sameState), dropping the
        // compute-write -> compute-read RAW dependency on the pyramid.
    }

    void DepthPyramidPass::ExecutePass(CommandBuffer *cmd)
    {
        if (!m_pyramid || !m_depth || !m_copySet)
            return;
        ...
        // UAV write->read hazard between consecutive levels (whole pyramid stays GENERAL).
        ...
        cmd->BeginDebugRegion("DepthPyramidPass");

        // mip 0: reduce the scene depth buffer (sampled) into the full-res pyramid level.
        ...
        // mips 1..N-1: reduce the previous pyramid level (storage source / storage dest).
        if (mips > 1)
    ...
    void DepthPyramidPass::Resize(uint32_t width, uint32_t height)
    {
        (void)width;
        (void)height;
        AcquireTargets();
    ...
    void DepthPyramidPass::Destroy()
    {
        ...
        if (SceneRendererHost *rs = GetActiveSceneRendererHost())
            rs->DestroyRenderTarget("depthPyramid");
        m_pyramid = nullptr;
        m_depth = nullptr;
    }

    std::vector<PassInfo *> DepthPyramidPass::GetPassInfos() noexcept
    {
