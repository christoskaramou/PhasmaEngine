...
namespace pe
{
    ...
    void SSRPass::UpdatePassInfo()
    {
        ...
        m_passInfo->pFragShader = Shader::Create({.sourcePath = Path::RuntimeAssets + "Shaders/SSR/SSRPS.hlsl", .entryPoint = "mainPS", .stage = PE_SHADER_STAGE_FRAGMENT, .defines = std::vector<Define>{}});
