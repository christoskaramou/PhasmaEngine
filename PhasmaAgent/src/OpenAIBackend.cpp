...
namespace pagent
{
            ...
            for (const auto &tc_delta : delta["tool_calls"])
            {
                ...
                if (tc_delta.contains("id"))
                {
                    ...
                    if (tc_delta.contains("function"))
                        acc.name = tc_delta["function"].value("name", std::string{});
                ...
                if (tc_delta.contains("function"))
                    acc.arguments += tc_delta["function"].value("arguments", std::string{});
            }
        }
    ...
    NeutralMessage OpenAIBackend::FormatToolResult(
