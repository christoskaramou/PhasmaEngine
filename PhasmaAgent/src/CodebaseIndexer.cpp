    ...
    int CodebaseIndexer::Index(const IndexerConfig &config)
    {
        ...
        for (const auto &filePath : files)
        {
            ...
            if (!isBinary)
            {
                ...
                if (useAST)
                {
                    auto astChunks = ASTChunker::ChunkCpp(source, config.max_chunk_chars);
                    if (astChunks.empty())
                        useAST = false;
                    else
                    {
                        for (auto &ac : astChunks)
                        {
                            if (m_cancel.load())
                                break;
                            std::string header = ASTChunker::BuildMetadataHeader(rel, ac);
                            std::string content = SanitizeUTF8(header + "\n" + ac.content);
                            std::string id = "codebase_" + rel + ":" + std::to_string(ac.startLine);
                            if (m_bm25)
