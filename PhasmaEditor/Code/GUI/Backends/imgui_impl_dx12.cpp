...
static bool ImGui_ImplDX12_IsSrvHandleInHeap(ImGui_ImplDX12_Data *bd, D3D12_GPU_DESCRIPTOR_HANDLE handle)
{
    if (bd == nullptr || bd->pd3dSrvDescHeap == nullptr || handle.ptr == 0)
        return false;

    D3D12_DESCRIPTOR_HEAP_DESC heap_desc = bd->pd3dSrvDescHeap->GetDesc();
    ...
    const UINT64 heap_start = bd->pd3dSrvDescHeap->GetGPUDescriptorHandleForHeapStart().ptr;
    const UINT64 heap_end = heap_start + (UINT64)heap_desc.NumDescriptors * descriptor_size;
    if (handle.ptr < heap_start || handle.ptr >= heap_end)
        return false;

    return ((handle.ptr - heap_start) % descriptor_size) == 0;
}
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (!ImGui_ImplDX12_IsSrvHandleInHeap(bd, texture_handle))
                    continue;
