        ...
        LightBindings()
        {
            ScriptSystem::AddBindings([](sol::state &lua)
                ...
                lights_table.set_function("add_point", []() {
                    GetActiveScene()->CreatePointLight();
                });
                lights_table.set_function("add_directional", []() {
                    GetActiveScene()->CreateDirectionalLight();
                });
                lights_table.set_function("add_spot", []() {
                    GetActiveScene()->CreateSpotLight();
                });
                lights_table.set_function("add_area", []() {
                    GetActiveScene()->CreateAreaLight();
                });
                ...
                lights_table.set_function("get_point_lights", [](sol::this_state ts) -> sol::as_table_t<std::vector<sol::table>> {
                    ...
                    auto &pls = GetActiveScene()->GetPointLights();
                ...
                lights_table.set_function("get_directional_lights", [](sol::this_state ts) -> sol::as_table_t<std::vector<sol::table>> {
                    sol::state_view lua(ts);
                    std::vector<sol::table> result;
                    auto &dls = GetActiveScene()->GetDirectionalLights();
                ...
                lights_table.set_function("get_spot_lights", [](sol::this_state ts) -> sol::as_table_t<std::vector<sol::table>> {
                    sol::state_view lua(ts);
                    std::vector<sol::table> result;
                    auto &sls = GetActiveScene()->GetSpotLights();
                ...
                lights_table.set_function("get_area_lights", [](sol::this_state ts) -> sol::as_table_t<std::vector<sol::table>> {
                    sol::state_view lua(ts);
                    std::vector<sol::table> result;
                    auto &als = GetActiveScene()->GetAreaLights();
