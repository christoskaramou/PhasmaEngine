...
- add shared frame command/semaphore primitives in `SceneFrameResources.*`. Editor and player both use the same helpers to wait/return frame command buffers and create/destroy acquire/submit semaphore vectors, while runtime resize still owns frame-resource recreation and editor resize keeps its existing behavior;
