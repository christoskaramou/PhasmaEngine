...
PhasmaEngine still renders 2D gameplay through the normal 3D scene path, but the runtime now has a small script-facing layer for simple 2D games. Cameras can be switched between perspective and orthographic projection, with projection mode and orthographic size serialized in `.pescene` camera data. Orthographic cameras skip projection jitter so 2D scenes stay stable under global TAA settings. The editor camera widget exposes the projection mode and keeps ImGuizmo in orthographic mode when the active camera is orthographic.

Lua can assign material textures at runtime with `material.set_texture(node, path)` or `material.set_texture(node, type, path)`, where `type` accepts base-color aliases such as `base_color`, `albedo`, and `diffuse`. Texture loads are synchronous helpers intended for setup/load-time script work rather than per-frame gameplay loops. The `sprite` Lua table wraps the current quad/material path: `sprite.create(path, width, height)` or `sprite.create({ texture = path, width = w, height = h, position = vec3(...), tint = vec4(...) })` creates a tagged sprite node. Sprites can also opt into screen-relative placement with `space = "ndc"`, `ndc_position = vec2(x, y)`, `ndc_size = vec2(w, h)`, and `depth = d`; NDC X/Y use the camera's normalized device range, NDC size uses the same units, and depth is world distance from the active camera. Sprites default to `alpha_cut`, which works with the current deferred path and common cutout textures; callers can pass `render_type = "opaque"`, `"alpha_cut"`, or `"alpha_blend"` and can change it later with `sprite.set_render_type(node, type)`. `sprite` tint also updates emissive color so quick 2D prototypes are visible without scene lighting.

Sprite nodes are still mesh-backed quads, but they now carry a `Component_Sprite` tag plus helpers for size, UV rectangles, atlas frame selection, simple frame animation, and world/NDC coordinate space. Lua exposes `sprite.set_space`, `sprite.set_ndc`, `sprite.set_uv_rect`, `sprite.set_frame`, `sprite.play_animation`, `sprite.stop_animation`, `sprite.update_animation`, and `sprite.update_all`; scene update advances playing sprite animations and refreshes NDC sprites against the active camera automatically. The editor can create sprites from the hierarchy Add menu, the hierarchy background context menu, and the Properties Add Component menu, and selected sprite nodes expose a Space selector plus atlas/frame, UV, FPS, loop, and play/stop controls. Sprite placement and ordering use the normal Transform widget; in NDC mode, Transform position X/Y are NDC coordinates, position Z is camera depth, and scale X/Y is NDC size. This is intentionally a thin helper over primitives and materials, not a native sprite batching or tilemap renderer.
...
- add shared scene sky resource helpers in `SceneSky.*`. Editor and player both use the same default day/night skybox plus IBL BRDF LUT load/destroy path, while editor-only `m_skyBoxWhite` stays editor-owned;
