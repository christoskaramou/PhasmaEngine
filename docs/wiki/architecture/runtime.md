...
The editor also hosts the same `RuntimeUiSystem` and Lua `runtime_ui.*` surface, but through an editor-side adapter that reuses the editor ImGui context. The adapter reapplies the shared runtime ImGui style/window defaults around each runtime screen and restores the editor style afterward. Runtime UI screens are positioned over the Viewport scene image and capture only their own overlay input before gameplay/script polling, so unrelated editor panels keep normal ImGui behavior. When editor chrome is hidden, the runtime UI keeps the GUI pass alive and falls back to the full window surface so Alt+G does not hide game UI. RmlUi is reserved as a future backend that can plug into the same lifecycle/event/render surface without rewriting runtime/game call sites.
