...
namespace pwgpu
{
    namespace
    {
        ...
        void HashString(Sha256 &hash, std::string_view value)
        {
        ...
        void HashWideString(Sha256 &hash, std::wstring_view value)
        {
            hash.UpdateUint64(value.size());
        ...
        std::string HexString(const std::array<uint8_t, 32> &digest)
        {
                                                ...
                                                const std::string &dxcVersion)
        {
            ...
            Sha256 hash;
            HashString(hash, "PhasmaWebGPU.DX12.DXIL");
            hash.UpdateUint32(kDx12WebGPUDxilCacheVersion);
            hash.UpdateUint32(kDx12WebGPUCbvRegisterShiftVersion);
            HashString(hash, kDx12WebGPUNagaBridgeVersion);
            HashString(hash, dxcVersion);
            HashString(hash, stage ? stage : "");
            HashString(hash, entryPoint ? entryPoint : "");
            HashWideString(hash, profile ? std::wstring_view(profile) : std::wstring_view());
