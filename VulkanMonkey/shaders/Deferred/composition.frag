...
vec3 calculateShadow(int i, vec3 fragPos, vec3 normal, vec3 albedo, float specular, float shadow)
{
	...
	// Diffuse part
	vec3 diff = ubo.lights[i].color.rgb * albedo.rgb * ubo.lights[i].color.a;
...
vec3 calculateColor(int i, vec3 fragPos, vec3 normal, vec3 albedo, float specular)
{
	...
	vec3 diff = ubo.lights[i].color.rgb * albedo.rgb * LdotN;
...
void main() 
{
	...
	vec3 fragColor = calculateShadow(0, fragPos.xyz, normal, albedo.rgb, specular, shadow);
	for(int i = 1; i < NUM_LIGHTS; ++i)
		fragColor += calculateColor(i, fragPos.xyz, normal, albedo.rgb, specular);

	// Ambient
	fragColor += 0.05 * albedo.rgb;
